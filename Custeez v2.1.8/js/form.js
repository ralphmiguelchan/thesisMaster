...
function viewForm(){
	...
	$.get("scripts/getforms.php?id=" + uid,function(data,status){
		...
		$.each(datum,function(i,item){
			...
			lmain.append("<div class='col-sm-4 notpend' style='margin:10px;width:30%;'><center><div id='proc_" + item.form_id + "'></div></center></div>");
			var rmain = $("#proc_" + item.form_id);
			
			rmain.append("<span>Name: <font style='color:#88d317'>" + item.formName + "</font></span><br>");
			rmain.append("<img src='img/form-icon.png' width='50%'></img><br>");
