...
function getGroups(){
	$.get("scripts/getgroups.php?gid=" + gid,function(data){
		...
		$.each(datum,function(i,item){
			...
			lmain.append("<div class='col-sm-4 notpend' style='margin:10px;width:30%;'><center><div id='proc_" + item.process_id + "'></div></center></div>");
			...
			rmain.append("<span><font style='color:#bc76b2'>" + item.processDetails + "</font></span><br>");

			rmain.append("<img src='img/process-icon.png' width='50%'></img><br>");
			rmain.append("<a href='editor.php?pid=" + item.process_id + "'><button type='text' class='btn btn-dash'>View</button></a>");
