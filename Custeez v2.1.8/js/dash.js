...
function FillPendingForm(){
	...
	$.get("scripts/getpendingform.php?uid=" + uid,function(data){
		...
		$.each(json,function(i,item){
			$("#notif").find(".row").append('<div class="col-sm-4">' +
					'<div class="notpend">'+
					'<center>'+
					...
					'<img src="img/form-icon.png" width="60%"><br>' +
					'<button type="text" class="btn btn-dash" onClick="ViewForm(' + item.sub_id + ');">View</button>' +
					'</center></div></div>');
		});
	});
}
...
function FillAccDec(){
	...
	$.get("scripts/accdecform.php?uid=" + uid,function(data){
		...
		$.each(json,function(i,item){
			if(item.substatus == 1){
				$("#appdecform").find(".row").append('<div class="col-sm-4">' +
						'<div class="notpend">'+
						'<center>'+
						...
						'<img src="img/form-icon.png" width="60%"><br>' +
						'<button type="text" class="btn btn-dash" onClick="ViewForm2(' + item.sub_id + ');">View</button>' +
				'</center></div></div>');
	...
	$.get("scripts/getpendingprocess.php?uid=" + uid,function(data){
		...
		$.each(json,function(i,item){
			$.get("scripts/countproc.php?pid=" + item.process_id,function(datax){
				$("#pendproc").find(".row").append('<div class="col-sm-4">' +
				'<div class="notpend">'+
				'<center>'+
				...
				'<img src="img/process-icon.png" style="height: 50%;"><br>' +
				'<div id="p_' + item.rgid + '"></div>' +
				'<a href="viewprocess.php?pid=' + item.process_id + '"><button type="button" class="btn btn-dash">View</button></a>' + 
				'</center>' +
				'</div>' +
				'</div>');
